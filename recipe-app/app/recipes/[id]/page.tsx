...
import { Clock, Users, PhilippinePeso, CircleCheck, Circle, ArrowLeft } from "lucide-react";
...
export default function RecipeDetail() {
  ...
  if (loading) {
    return (
      <main className="min-h-dvh flex items-center justify-center bg-neutral-200 sm:bg-neutral-300">
  ...
  return (
    <main className="min-h-dvh flex items-start justify-center bg-neutral-200 sm:bg-neutral-300 sm:py-6">
      ...
      >
        {/* Sticky header */}
        <header
          className="sticky top-0 z-20 flex items-center gap-3 px-4 sm:px-5 pt-5 pb-3.5 backdrop-blur-md shadow-sm"
        ...
        >
          ...
          <p className={`text-lg sm:text-xl font-semibold ${poppins.className}`} style={{ color: "#F5EFE0" }}>
            {recipe.name}
          </p>
        </header>

        {/* Meta row */}
        <div className={`flex gap-4 px-5 pt-3 pb-4 text-xs sm:text-sm items-center ${poppins.className}`} style={{ color: "#D8CFC0" }}>
          {recipe.prep_time && (
        ...
        <div className="flex-1 px-4 pb-8 space-y-3.5 overflow-y-auto">
          {/* Ingredients card */}
          <div className="rounded-2xl p-4" style={{ backgroundColor: "#F5EFE0", color: "#3D2E1F" }}>
