...
function RecipeCard({
  ...
  return (
    ...
    >
      ...
      <Link
        href={`/recipes/${recipe.id}?have=${encodeURIComponent(
          haveIngredients.join(",")
        )}&servings=${servings}`}
...
export default function ResultsContent() {
  ...
  const budget =
    !isNaN(rawBudget) && rawBudget >= 0 ? rawBudget : 0;

  const rawServings = Number(searchParams.get("servings"));
  const servings =
    !isNaN(rawServings) && rawServings >= 1
      ? Math.floor(rawServings)
      : 1;
      ...
      const matched: RecipeResult[] = data.map((recipe) => {
        ...
        recipe.recipe_ingredients.forEach((ri) => {
          ...
          if (
            ...
            haveIngredients.includes(ingredientName)
          ) {
            haveList.push(ingredientName);
          } else {
            missingList.push({
              name: ingredientName,
              price: calculateIngredientCost(
                ingredient.price,
                ri.quantity,
                ri.unit,
                ingredient.piece_weight_kg,
                servings
              ),
            });
          }
        ...
        const costToCook = missingList.reduce(
    ...
    <main className="min-h-dvh flex items-start justify-center bg-neutral-200 sm:bg-neutral-300 sm:py-6">
      ...
      >
        ...
        <header
          ...
          style={{
            backgroundColor: "rgba(92, 107, 61, 0.96)",
          }}
        >
          <button
            onClick={() => router.push("/")}
          ...
          <div>
            <h1
              className={`text-lg sm:text-xl font-semibold ${poppins.className}`}
              style={{ color: "#F5EFE0" }}
            >
            ...
            >
              {loading
                ? "Finding recipes..."
                : `${affordable.length} recipe${
                    affordable.length !== 1 ? "s" : ""
                  } fit your ₱${budget} budget`}
            </p>
          </div>
        </header>

        <p
          className="text-xs sm:text-sm px-5 pt-2 pb-3 text-justify leading-relaxed"
          style={{ color: "#D4DEBC" }}
        >
          Common pantry items (cooking oil, soy sauce, vinegar, salt, and
          pepper) are assumed available and are not included in the final
          cost.
        </p>
        ...
        <div className="flex-1 px-4 pb-8 space-y-3">
          {loading ? (
            ...
            <p className="text-sm text-white/80 px-2">
              No recipes fit that budget with your current ingredients yet
              — try raising your budget or adding more ingredients you
              have.
            </p>
          ) : (
            <>
              ...
              {closeCalls.length > 0 && (
                <>
                  <p
                    className={`text-xs ${poppins.className} px-1 pt-2`}
                    style={{ color: "#D4DEBC" }}
                  >
